...
public class BlockChainActionInitial implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (actionName.equals(Eth_CreateAddress.class.getSimpleName())) {
            future = scheduler.submit(new Eth_CreateAddress(ethWalletService));
        } else if (actionName.equals(Eth_AnalyzeBalance.class.getSimpleName())) {
            future = scheduler.submit(new Eth_AnalyzeBalance(ethWalletService));
            ...
            future = scheduler.submit(new Eth_Transfer(ethWalletService, ja.toArray()));
            ...
            future = scheduler.submit(new Eth_CollectMain(ethWalletService, type));
        } else if (actionName.equals(Eth_Obfuscate.class.getSimpleName())) {
            future = scheduler.submit(new Eth_Obfuscate(ethWalletService));
        }
